...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='The config of the Dataset, leave as None if theres only one config.',
    )
    parser.add_argument(
...
def get_full_repo_name(model_id, organization, token):
    if token is None:
    ...
    if organization is None:
        ...
        repo_name = f'{organization}/{model_id}'
    return repo_name
...
def main():
    ...
    def is_not_buffer(name, params_dict):
        return name in params_dict
    
    def vectorize_and_ignore_buffers(g, params_dict=None):
        '''
        gradients are given as a tuple (grad_w0, grad_w1, ... grad_wp) where
        p is the number of weight matrices. each grad_wi has shape
        [batch_size, ...] this function flattens g to have shape
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        for index_t, t in enumerate(selected_timesteps):
            ...
            # ft_per_sample_grads = vectorize_and_ignore_buffers(list(ft_per_sample_grads.values()), params)
